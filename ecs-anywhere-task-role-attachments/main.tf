resource "aws_iam_policy" "push_to_s3" {
  count       = length(var.allowed_s3_arns)
...
resource "aws_iam_policy" "read_and_delete_from_s3" {
  count       = length(var.allowed_read_and_delete_s3_arns)
...
resource "aws_iam_role_policy_attachment" "kms-to-ext-machine-user" {
  count      = length(var.allowed_kms_arns)
  role       = var.task_role_name
  policy_arn = aws_iam_policy.encrypt_with_kms[0].arn
}

resource "aws_iam_role_policy_attachment" "decrypt-kms-to-ext-machine-user" {
  count      = length(var.allowed_decrypt_kms_arns)
  role       = var.task_role_name
...
resource "aws_iam_role_policy_attachment" "s3-to-ext-machine-user" {
  count      = length(var.allowed_s3_arns)
  role       = var.task_role_name
...
resource "aws_iam_role_policy_attachment" "s3-read-to-ext-machine-user" {
  count      = length(var.allowed_read_s3_arns)
  role       = var.task_role_name
...
resource "aws_iam_role_policy_attachment" "s3-read-and-delete-to-ext-machine-user" {
  count      = length(var.allowed_read_and_delete_s3_arns)
  role       = var.task_role_name
...
resource "aws_iam_policy" "push_to_sqs" {
  count       = length(var.allowed_sqs_arns)
...
resource "aws_iam_role_policy_attachment" "sqs-to-ext-machine-user" {
  count      = length(var.allowed_sqs_arns)
  role       = var.task_role_name
...
resource "aws_iam_policy" "pull_from_sqs" {
  count       = length(var.allowed_pull_sqs_arns)
...
resource "aws_iam_role_policy_attachment" "pull-sqs-to-ext-machine-user" {
  count      = length(var.allowed_pull_sqs_arns)
  role       = var.task_role_name
...
resource "aws_iam_policy" "temporary_queue_response_sqs" {
  count       = length(var.allowed_temporary_queue_response_sqs_arns)
...
resource "aws_iam_role_policy_attachment" "push-sqs-to-ext-machine-user" {
  count      = length(var.allowed_temporary_queue_response_sqs_arns)
  role       = var.task_role_name
...
resource "aws_iam_role_policy_attachment" "create-temporary-queues-sqs-to-ext-machine-user" {
  count      = length(var.allowed_temporary_queue_request_sqs_arns)
  role       = var.task_role_name
...
resource "aws_iam_role_policy_attachment" "read_ssm_config_to_ext_machine_user" {
  count      = length(var.allowed_ssm_arns) > 0 ? 1 : 0
  role       = var.task_role_name
...
# Attach policy to ext user.
resource "aws_iam_role_policy_attachment" "push_metric_data_config_to_ext_machine_user" {
  count      = var.cloudwatch_metrics_access ? 1 : 0
  role       = var.task_role_name
...
##############################################################################
# Allow Publish to SNS topics.
##############################################################################
resource "aws_iam_policy" "publish_to_sns" {
  count       = length(var.allowed_publish_sns_arns)
  name_prefix = "${var.name_prefix}-publish-to-sns"
  description = "This policy allows publihs access to specific SNS topics"
...
data "aws_iam_policy_document" "publish_to_sns" {
  ...
  statement {
    ...
    resources = concat(var.allowed_publish_sns_arns, formatlist("%s/*", var.allowed_publish_sns_arns))
  }
}

resource "aws_iam_role_policy_attachment" "sns-publish-to-task-role" {
  count      = length(var.allowed_publish_sns_arns)
  role       = var.task_role_name
  policy_arn = aws_iam_policy.publish_to_sns[0].arn
}
